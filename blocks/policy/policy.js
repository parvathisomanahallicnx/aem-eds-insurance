    ...
    [...row.children].forEach((col) => {
      ...
      if (pic) {
        ...
        // picture is only content in column
          picWrapper.classList.add('policy-img-col');
        }
      } else {
        col.classList.add('policy-content');
        const childDiv = document.createElement('div');
        childDiv.innerHTML = col.innerHTML;
        childDiv.className = 'policy-subcontent';
        ...
        const heading = childDiv.querySelector('p');
        heading.classList.add('policy-heading');
      }
    });
  });
}
